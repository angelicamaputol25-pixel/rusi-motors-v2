// ============================================================
//  RUSI MOTORS V2 – Authentication + Role Module
// ============================================================
...
export async function getUserRole(uid) {
  try {
    ...
    return sessionStorage.getItem('rusi_v2_role') || 'Staff';
  }
}

export async function cacheUserRole(uid) {
  const role = await getUserRole(uid);
  sessionStorage.setItem('rusi_v2_role', role);
  return role;
}
...
export function requireAuth() {
  ...
  onAuthStateChanged(auth, async (user) => {
    if (!user) {
      ...
      if (!sessionStorage.getItem('rusi_v2_role')) await cacheUserRole(user.uid);
      applyRoleUI(sessionStorage.getItem('rusi_v2_role') || 'Admin');
...
export function populateSidebarUser() {
  onAuthStateChanged(auth, async (user) => {
    if (!user) return;
    let role = sessionStorage.getItem('rusi_v2_role');
...
export function applyRoleUI(role) {
  const h = { Admin: 4, Manager: 3, Staff: 2 };
