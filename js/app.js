...
export function startClock(elementId = 'live-clock') {
  ...
  if (!el) return;
  const tick = () => { el.textContent = new Date().toLocaleString('en-PH', { weekday:'short', month:'short', day:'2-digit', hour:'2-digit', minute:'2-digit', second:'2-digit', hour12:true }); };
...
export class Paginator {
  constructor(items, perPage = 15) { this.items = items; this.perPage = perPage; this.current = 1; }
  ...
  renderControls(containerId, onPageChange) {
    ...
    el.innerHTML = `
      <span class="text-secondary" style="font-size:0.75rem;font-family:'Rajdhani',sans-serif;font-weight:600;letter-spacing:0.5px">PAGE ${this.current} / ${this.totalPages||1} &nbsp;•&nbsp; ${this.items.length} RECORDS</span>
